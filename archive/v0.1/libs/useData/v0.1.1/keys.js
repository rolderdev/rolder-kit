...
const keys = {
    ...
    dbClass: (dbClass) => [{ ...keys.all()[0], dbClass }],
    command: (command, dbClass) => [{ ...keys.dbClass(dbClass)[0], command }],
    fetch: (props) => [{
        ...
        options: props.options ? props.options[0] : Rolder?.classes[props.dbClass].defaultOptions,
    }],
    customFetch: (props) => [{
        ...keys.command('customFetch', props.dbClass)[0],
        query: props.query ? props.query[0] : {},
        sort: props.sort ? props.sort[0] : Rolder?.classes[props.dbClass].defaultSort,
        options: props.options ? props.options[0] : Rolder?.classes[props.dbClass].defaultOptions,
    }],
