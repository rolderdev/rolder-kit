...
const {
  ...
  resetSelected, doDelete, createData, createdItems, qrCodeLevel, doViewImages, createUser, createdUser, dbClass, autoHeight,
} = sharedProps
...
const props = {
  ...
  createUser: {
    '0.1.0': {
      ins: { createUser },
      outs: { isCreating, sendCreated, createdUser },
    }
  },
  useData: {
    '0.1.1': {
  ...
  UseData: {
    '0.2.0': {
      ins: { useDataType, useDataEnabled, dbClass, setRefs, refMap, query, sorts, options, id, ids },
      outs: { isLoading, sendLoaded },
      dyn: [
        { condition: 'setRefs = true', inputs: ['refMap'] },
        { condition: 'useDataType = fetch', inputs: ['query', 'sorts', 'options'] },
  ...
  Box: {
    '0.1.0': {
      ins: { ...sxDimensions, autoHeight, bottomOffset },
      dyn: [
        { condition: 'autoHeight = true', inputs: ['width', 'bottomOffset'] },
        { condition: 'autoHeight = false', inputs: ['width', 'height'] },
      ],
    }
  },
  stack: {
  ...
  Button: {
    '0.1.1': {
      ins: { ...margins, ...paddings, disabled, size, radius, color, iconSize, iconName, isLoading, variant: { ...actionVariant }, value, type: buttonType },
      outs: { sendClicked }
    }
  },
  actionIcon: {
  ...
  Drawer: {
    '0.1.1': {
      ins: { position: drawerPosition, title, size, show, withCloseButton },
      outs: { sendHided }
    }
  },
  Modal: {
