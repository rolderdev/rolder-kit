...
//@ts-ignore
const PasswordInput = lazy(() => import('@packages/password-input'))
            ...
            <Aside features={['Custom props', 'Props function']} scopes='Table'>
                <Comp>
                    <PasswordInput
                        {...p}
                        value={p.value}
                        onChange={(e: any) => setProps((prev: any) => ({ ...prev, value: e.target.value }))}
                    />
                </Comp>
                <Configurator w={340}>
                    <TextInput
                        ...
                        onChange={v => setProps((prev: any) => ({ ...prev, label: v.target.value }))}
                    />
                    <TextInput
                        label='Placeholder (placeholder)'
                        default={defaults.placeholder}
                        onChange={v => setProps((prev: any) => ({ ...prev, placeholder: v.target.value }))}
                    />
                    <Group noWrap>
                        <SizeSlider
                            name='Radius (radius)'
                    ...
                    <SizeUnit
                        ...
                        defaultSize={100}
                        defaultUnit='%'
                        onChange={(size, unit) => setProps((prev: any) => ({ ...prev, w: size + unit }))}
                    />
                    <Switch
                        label='With asterisk (withAsterisk)'
                        default={defaults.withAsterisk}
                        onChange={v => setProps((prev: any) => ({ ...prev, withAsterisk: v.target.checked }))}
                    />
                    <Switch
                        label='Disabled (disabled)'
                        default={defaults.disabled}
                        onChange={v => setProps((prev: any) => ({ ...prev, disabled: v.target.checked }))}
                    />
                    <Switch
                        label='Error (error)'
                        default={false}
                        onChange={v => setProps((prev: any) => ({ ...prev, error: v.target.checked }))}
                    />
                    <TextInput
                        label='Error (error)'
                        onChange={v => setProps((prev: any) => ({ ...prev, error: v.target.value }))}
                    />
