...
const UseData = lazy(() => import('@shared/use-data'))
const defaults = {
    fetchScheme: [
        {
            dbClass: 'car',
            ...
            <Configurator w={600}>
                ...
                {mounted
                    ? <Switch
                        ...
                        default={false}
                        onChange={v => setProps(prev => ({ ...prev, searchEnabled: v.target.checked }))}
                    />
                    : null}
                {p.searchEnabled
                    ? <TextInput
                        placeholder='Поиск'
                        onChange={v => setProps(prev => ({ ...prev, searchString: v.target.value }))}
                    />
                    : null}
                {mounted
                    ? <Switch
                        ...
                        default={false}
                        onChange={v => setProps(prev => ({ ...prev, paginationEnabled: v.target.checked }))}
                    />
                    : null}
                {p.paginationEnabled
                    ? <Group grow>
