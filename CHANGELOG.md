...
* RK переведен на [Turborepo](https://turbo.build/repo). Нужно для удобства разработки. Позволит более точно сделовать принципо Monorepo.
* С этой версии начинаем вводить управление стилями глобально средствами Mantine. Для этого обновлена нода Mantine, в которой появмлся параметр управления темой. Сходу вылезла проблема - у наших компонентов заданы дефолтные параметры, которые перезаписывают параметры, заданные в теме Mantine. Будем изменять поведение дефолтных настроек под этот сценарий. Подробнее в port-fabric.
...
* Добавлена настройка [темы](https://v6.mantine.dev/theming/theme-object/). Сама тема задается на уровне проекта, т.к. нужна до загрузки ноды Mantine для установки верных дефолтных значений портов.
...
* Параметр Сolor scheme изменился и теперь имеет три варианта:
  * Light - светлая тема и дефолтное значение.
  * Dark - темная тема.
  * Auto - тема системы.
* В режиме Auto управлять темой нельзя, но можно получать Color scheme на выходе.
* Когда не Auto, можно:
...
* `#breakingChange` Убран параметр Detect color scheme.
* `#breakingChange` Убран параметр Сolor scheme.
* Настройки темы вставляются из настроек проекта.
