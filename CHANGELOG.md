...
* Проверяем гипотезу составной, модульной таблицы. Начнем с базового, постепенно добавим фильтры, сортировки, действия в ячейках и т.д.
* В отличии от версий 1.х эта таблица всегда server-side, т.е. фильтры и сортировки должны выполнятся на уровне UseData.
* Построена на библиотеке, с котороый мы начинали - https://icflorescu.github.io/mantine-datatable-v6
* Как старые версии таблица умеет форматировать данные.
* Принцип модульности означает, что нет вариантов для группировки или иерархии. Таблица всегда просто таблица. Если нам нужна группировка или иерархия, мы организуем ее на уровне данных.
* Модули. Таблица работает с 3-мя видами модулей. 1 встроенный - колонки. Они настраиваюстя через схему, специальные колонки заготовлены в самой ноде и выбираются по названию accessor. 2 вставляемые - вставляются как дочерние элементы в таблицу.    
    * Колонки:
        * Данные. Стандартная колонка, которая отрисовывает данные в заданом формате.
        * Чекбокс. Включается настройкой miltiselect.
        * Expander. Колонка с шевроном. Шеврон можно сделать кликабельным. Всключается через accessor = 'Expander'.
    * Вставляемые модули. Принцип работы такой, учитывая, что дочерним модулем не может быть массив, только один модуль. Таблица считывает поданные в нее дочерние модули, отфильтровывая по имени нод специальные модули. Если в результате остается один модуль, он передается как дочерний, иначе игнорируется. Специальные модули не подаются как дочерние, а используются в соответствии с настройками схемы:
        * Стандартный модуль. Любой дочерний элемент, в том числе и новая таблица.
        * Специальные модули:
            * TableFilters. Отдельная нода, в которую можно обернуть любые другие ноды. Дочерние ноды вызываются нажатием на иконку фильтра. Дальше вся логика решается этой нодой. В схеме дочерние ноды привязываются по порядковому номеру.
            * TableActions. Отдельная нода, в которую можно обернуть любые другие ноды. В схеме дочерние ноды привзяываютя по порядковому номеру к отдельной колонке или ячейке.
* Порты:
    * Входные:
        ...
        * Размеры:
            * Width - ширина.
            * Max height - макимальная допустимая высота.
        ...
        * Данные:
            * Default selected item - дефолтный выбранный item.
            * Default sorts - дефотные сортировки. Таблица не может по данным определить как они отсортированы, ей нужно об этом сказать.
        * Выбор:
            * Selectable - включить выбор.
            * Single row - выбор одной строки.
        * Стиль таблицы:
        ...
        * Стиль строк:
            ...
            * Background color - цвет строки.
            * Highlight on hover - подсвечивать при наведении.
            * On hover bg color - цвет при наведении.
            * Selected bg color - цвет выделения.
    * Выходные:
        * Selected item - выбранный item.
        * Sorts - выбранные сортировки в формате, который можно подавать в UseData.
