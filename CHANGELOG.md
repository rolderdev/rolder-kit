...
## 2024-08-03 v1.0.0-rc3

### Внутренние изменения RK

- Установлен и настроен [Rsdoctor](https://rsdoctor.dev/) - анализатор кода и пакетов. RK давно не маленький (11MiB zipped), нужно бороться с размерами. Для использования нужно добавить `RSDOCTOR=true` перед командой `moon`. Для каждого пакета откроется своя стрница в браузере, где можно изучить анализ, проведенный доктором. Билд будет долгим, но информативным.
- Как следствие - чистка дублей разных версий библиотек.

### data

#### UseData v1.4.0 `#experimental`

Добавлена версия API. Версия API - это версия контроллера на сервере Kuzzle. Сделано, чтобы можно было менять схему UseData с поддержкой обратной совместимости. v1 требует Kuzzle v0.9.0, v0 для старых версий.

### mantine

#### Paper v2.0.0 `#pre-release`

- Переведен на Mantine v7.
- Добавлена возможность отключать тень.
- Порты выстроены в более логичном порядке.

#### [Table v2.0.0](https://docs.rolder.app/#/table) `#experimental`

Начал [документировать](https://docs.rolder.app/#/table).

##### Добработки

- Сортировка. Пока без TableScope.
- Идея с автоматическим отступом провалилась. Слишком много вариантов. Теперь это отдано на откуп разработчику через функцию `Left padding func`. Функция подает уровень таблицы в иерархии и item текущей строки. Нужно возвращать число или строку в формате, который понимает параметр `pl` Mantine, т.е. любой приемлемый `CSS` или относительные размеры (rem, sm, md и т.д.).

##### Испралвенные баги

- Огромное количество лишних рендеров ячеек из-за бага в последней версии библиотеки, откатился на предыдущую.
- Не завершалась анимация загрузки, если подать пустой массив items.
- Не устанавливался цвет строки.

## 2024-07-31 v1.0.0-rc2

**_Для использования этой версии нужно обновить Data до версии 2.0.0_**

### Общее

- Почищен README.
- Удалена неиспользуемые внтуренняя утилита `getGuid`.
- Rspack v1.0.0-beta1. Еще в первых версиях RK была сделана ставка на новый (и китайский) Rspack вместо традиционного WebPack. Причина проста - сборка RK быстрее раз в 10. И вот ребята подошли вплотную к первой версии. Просто следуем за ними, чтобы потом не оказалось, что нужно переделывать пол проекта.

### app

#### App v2.0.0 `#pre-release`

- Поправлен баг, из-за которого приложение сваливалось, если логи выключены.
- Добавлена информация о пользовтеле в HyperDX.
- Обнаружил, что HyperDX а отличии от Sentry видит ошибки в функциях Noodl.

### mantine

#### Table v2.0.0 `#experimental`

- Исправлено неверное поведение - не работал отступ в дочерней таблице, если в ней был выключен expansion. Для этого параметр отсупа перенесен в rowStyles из expansion. Называется `Level * left padding`, т.е. берется текущий уровень таблиц и умножается на значение. 0 - 0, 1 - 16, 2 - 32 и т.д. Под капотом таблица сама определит куда ставить отступ - в чекбокс, в шеврон или просто в ячейку.
- Убран не нужный параметр в схеме таблицы - expander. Была гипотеза, что будут сценарии, когда шеврон не нужен. Пока такого не было.

## 2024-07-31 v1.0.0-rc1

**_Для использования этой версии нужно обновить сервер Kuzzle до версии 0.8.0 и его настройки._**

### Общее

- Изменен способ загрузки R.dbClasses. Раньше это происходило только после авторизации. Такой вариант не походит, когда авторизация опциональна, как это сделано в Docs. Теперь R.dbClasses загружаются в Data при инициализации.
- Добавлена возможность хранить опции в бекенде. Сама идея давно работает для dbClasses, creds и давным давно проверена на прошлой инфраструктуре, когда был Parse, а не Kuzzle. Теперь можно добавить новый класс `option_v1`, в котором каждый документ - это опция. Структура простая: name - название опции, data - объект любой структуры со значением опции. При инициализации Data скачает эти опции и положит в `R.params.backendOptions`. Зачем это нужно? Чтобы можно было менять какие то параметры приложения без нового деплоя. Первый пример - запись логов. Если в опции добавить такой документа:
...
То Data включит/выключит запись логов при старте приложения.

Для обоих новшеств требуется изменить настройки доступа на сервере, чтобы анонимные пользователи могли брать данные из индекса `config` до авторизации специальным методом сервера:
...
```json
{
	"controllers": {
		...
		// Эта запись дает доступ к чтению в fetchConfig.
		"document": {
			"actions": {
				"search": true
			}
		},
		// Эта запись дает доступ к чтению конфига.
		"rolder": {
			"actions": {
				"fetchConfig": true
			}
...
- Убран Senrty. Проиграл HyperDX.
- HyperDX теперь включаться/отключаться с конфига в бекенде. Чтобы это работало нужно добавть `apiKey` от HyperDX в creds бекенда и настройку в options бекенда.

### data

#### Auth v2.0.0 `#pre-release`

- Больше не загружает R.dbClasses и R.params.creds. Теперь это делает Data. Соответсвенно, стали доступны варианты без авторизации и с опциональной авторизацией. Для опционального варианта Auth должен лежать в Data рядом с остальными компонентами приложения. Для варианта, когда авторизация обязательна все осталось как было - все содеражние проекта лежит в Auth.
- Переезд R.dbClasses в Data значит, что теперь при смене dbClass в конфиге бекенда не нужно перелогиниваться, достаточно обнвить приложение, чтобы Data скачала новые данные.

#### Data v2.0.0 `#pre-release`

- Добавлены скачивание конфига с бекенда. Требует Kuzzle v0.8.0
