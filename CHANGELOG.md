...
* Порты:
  ...
  * Filter (нода ColumnFilter):    
    ...
    * Set filter value - установить значение параметра фильтрации.
    * Filter - сигнал зупуска фронт-фильтрации и активации иконки при любом типе фильтрации.    
...
* Инструкции:
  ...
  * Кастомная ячейка:
    * Есть 2 режима работы - не контролируемый и контролируемый.
        * Не контролируемый.
            * Предпочтительный режим, т.к. быстрый и легккий в использовании. Внешне таблица себя ведет нативно.
            * Можно вставлять любые ноды, но та нода, что берет данные должна быть подготовлена. Сейчас сделана нода Text v0.6.0.
            * Нельзя взать текущий item, а значит нельзя реагировать и менять отображаемое в зависисоти от содеражния item, нельзя вставлять реакции, действия и т.д.        
        * Контролируемый. 
    ...
    * Дабы меньше использовать тяжелый контроллируемый режим, придумал новую фичу для нод - Props function. Идея проста - откуда бы не приходили данные, можно обработать их функций и изменить любые props. Минусы - нужно знать внтуреннее название настройки, лекго напартачить. Первая проба пера - Badge.
    * Инструкция:
  ...
  * Сотрировка:
    ...
    * Схема сортировки задается для каждой колонки с помощью ключа 'sort'. Наличие ключа включает сортировку. Когда не нужны параметры, но нужна сортировка ставим в значение true.
    * Бекенд:
      * Таблица никогда не сортирует данные.
      * Default - дефолтное направление сортировки. Может быть 'asc' или 'desc'. Может быть только у одной колонки, таблица возьмет из первой, если несколько. Устанавливаем тогда, когда из бекенда приходят уже первично отсоритрованные данные. Если ничего не отсортировано изначально, не устанавливаем.
  ...
  * Фильтрация:
    ...
    * Бекенд:
      * Преобразовать данные работы фильтра в Filters для UseData и передать в нее. UseData вернет новые данные, таблица обновится.      
    * Фронт:
      * Связать выходной сигнал ноды-фильтра с входным сигналом 'Filter' в ColumnFilter.
      * Настройка схемы. Для колонки с тем же индексом нужно указать ключ 'filter' и в нем ключ-функцию 'func', как это сделано с сортировкой. Возвращаемые items должны сохранять свою структуру. Результат может быть пустым массивом. Принимает 2 параметра - items и filterValue. Таблица возьмет текущиие items и поданный filterValue в функцию. В утилитах (R.utils) есть filterBy, может облегчить задачу. Пример:
        ```js
        [
            {
                accessor: 'content.name',
...
* Известные ограничения:
  * Сортировка:
    ...
    * Анимация уменьшаяет ширину колонок. Без анимации моргают кастомные ячейки.
  * Фильтрация:
...
***#### MultiSelect v0.3.0
* Нода пересобрана в такой же манаере как и Select - внутри это отдельные компоненты для вариантов с формой и без.
...
* Добавлен выходно сигнал, когда нода закрывается.
* Нода выдавала item и сигнал одновременно. Теперь всегда саначала item, потом сигнал.
* Нода не отрабатывала Default items.
***#### Text v0.6.0
* Добавлен источник данных - таблица.
***#### Badge v0.3.0
* Добавлен источник данных - таблица.
* 
