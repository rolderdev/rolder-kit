...
- Rspack научился использовать общий конфиг. Теперь разработчег RK может:
  ...
  - Запускать команды выше в указанный Roodl-проект `moon :build -- --env developer=decard project=startum-desktop`. Где developer - ник разработчика, project - название проекта. В папке developers должен лежать файл ${ник разработчика}.json с описанием проектов. В файле указывается projectsDir - папка, где разработчик держит свои Roodl-проекты и projects - массив проектов, в которых указаны название папок и список пакетов, которые должны копироваться в проект. Смотри пример `decard.json`.
...
- Логи. Теперь можно поднять сервер логов и читать ошибки постфактум, а не только со скринов и видео. Не умеет ловить ошибки в функциях Roodl, но можно отлавливать ошибки и отправлять вручную.
...
Добавлена новая нода LocalData, новые версии App и Auth для проверки гипотезы `Local-First`.
...
## 2024-06-07 v1.0.0-beta26-Vezdexod
