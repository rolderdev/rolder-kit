...
- Исправлен баг - билд в проект Roodl, билдил все пакеты, а не выбранные.
...
## 2024-08-09 v1.0.0-rc4

### Общие изменения RK

Пересобрал интеграцию с `Roodl`. Пока для новых JS-нод. Она состоит из двух пакетов - `node` и `port`. Первый регистрирует ноду в `Roodl`, обрабатывает порты (тут много всего), выдает ошибки в `Roodl`, задает стандарт для разработчика RK. Второй типизирует структуру порта и дает разработчику RK инструменты управления портами. Первый прожил 8 версий, воторой 6. Все гипотезы, что нужно было проверить, проверены. Новые версии - `1.0.0`. Внутренняя структура полегчала в пару раз. Убрано много лишних проверок и повторяемых функций. Убраны мелкие проблемы.
Есть изменения как для разработчика RK, так и для разработчка в `Roodl`.

#### Изменения для разработчика RK

- Больше нет `общих портов`. Практика показала, что это не всегда удобно, а главное, все общие порты, хоть и не всегда используются, но увеличивают вес пакетов.
- Список используемых нами `типов портов` устаканился. Разработчик не может установить тип порта, который не задан в стандарте.
- Наши `кастомные порты` так же устаканились. Их два - `funcEval` и `objectEval`. Оба функции, но первая остается функцией внутри компоненты, а вторая принимает текущие props, выполняется и передает результат в компоненту. Такой подход универсален и гибок.
- Обработка `зависимых портов` улучшилась. Раньше приходилось перечислять в зависимом порте все порты, которые сами имели зависимости. Т.е. повторять условие родительской зависимости.
- Больше нет опции `required`. Теперь разработчик сам должен задать функцию `validate` и сообщить разработчику `Roodl`, что не так.
- Вернулись `подсказки`, всплывающие при наведении на название параметра в редакторе. Оказалось, что туда можно передавать HTML-теги, а значит демонстрировать примеры, подробно что-то описывать. И все это с красотами вплоть до видео.
- Заново осмыслена и восстановлена функция `getInspectInfo`. Теперь можно выдавать какую то полезную информацию над нодой в `Roodl`. Функция принимает как значения входных портов, так и выходных. Информацию можно структурировать, наводя относительную красоту. Хороший пример - схема колонок в таблице.
- Добавлена новая функция на уровне порта - `transform`. Нужна для случаев, когда параметры порта нужно менять условно. На вход подает текущие props, нужно вернуть порт с новыми параметрами. Потребность возникла из-за портов-списков, где этот список нужно менять. Другой вариант - когда нужно устанавливать дефолт в зависимости от выбора другого порта. Важно не менять тип порта.
- Такой же `transform` доступен для всей ноды. Т.е. можно по каким то условиям менять состав и/или параметры портов. Это заменяет устаревшую `addNodePorts`.
- `customProps` теперь добавляется автоматически. Можно отключить через `disableCustomProps`.
- `JS-ноды` научились работать не только по сигналу, но и реактивно. Т.е. теперь можно делать такие JS-ноды, которые срабатывают на подачу конкретных инпутов.
- В `README` RK написал памятку для разработчика ноды. Пока в части декларации ноды.

#### Изменения для разработчика в Roodl

Обнаружил, что совсем не обязательно указывать массив в редакторе параметров `Roodl`. Мы долго использовали такой трюк - в параметрах задаем массив с единственным объектом или функцией. Под капотом массив трансформировался в объект или функцию, которые и использовались в ноде. Приходилось проверять, что массив с одним индексом и заниматься дополнительной конвертацией. Все это по одной причине - `Roodl` в параметрах ноды открывает на редактирование с проверкой JS-кода только массив. Оказалось - указываем, что массив, но пишем стрелочную функцию и работает. Скажем спасибо `Noodl`, который мало занимался проверкой типов портов.

Отсюда такие изменения, которые будут прилетать с обновлениями нод:

- Везде, где параметр - это функция, разработчик теперь использует обычную стрелочную функцию, не оборачивая это в массив и объект с ключем-функцией. Входные параметры, как обычно, определяет конкретная нода.
- Везде, где параметр предполагает что-то сложнее простых литералов, а именно передать объект - это так же функция, но на вход она подает текущие параметры ноды и просит вернуть структуру объекта, которая задается конкретной нодой.
- `Custopm props` теперь будет работать как раньше работала `Props function`. С той разницей, что нужно указывать стрелочную функцию.

### data

#### search v0.1.0 `#experimental`

Нода для поиска на фронте. Основана на [Fuse.js](https://www.fusejs.io). Умеет искать по простому массиву items. Это первая реактивная JS-нода. Планирую научить ее работать с иерархией.

### mantine

Обновлен до версии 7.12.0

#### [Table v2.0.0](https://docs.rolder.app/#/table) `#experimental`

##### Добработки

- Добавлена сортировка через TableScope. Нужно настраивать как обычную сортировку. Если включен Scope, то состояние сортировки подтянется с TableScope и содержимое синхронизируется у всех дочерних таблиц. В дочерних таблицах можно указывать сортировку по другому классу или ключу, но нужно установить одинаковое изначальное состояние.
- Добавлен новый режим для `On row click` - `Function`. В этом режиме появляется опция `Click func`. Это функция, которая как `getValue` подает item нажатой строки, все items таблицы и `hierarchyNode`, если есть иерархия. На выход функция ничего не ждет. Предполагается, что разработчик сам решает как это использовать. Очевидные решения - заполнять некую переменную Noodl нажатым item или запускать событие, передавая выбранный item (или все items, или hierarchyNode). Пример:

```ts
[(item, items, hierarchyNode) => Noodl.Events.emit('ViewSection#Specification', { clickedSection: item })];
```

Помимо `Click func` можно применять `Click filter func` ровно так же как это делается с режимом `Signal`. Это нужно, чтобы не показывать курсор-руку для отфильтрованных строк. Когда есть потребность в фильтрации, логично применять обе, чтобы не повторять условия в `Click func`. Сначала запускается `Click filter func`, потом `Click func`, если первая вернула `true`.

##### Испралвенные баги

- Разворачивание строк не работало без TableScope.

#### TableScope v0.1.0 `#experimental`

- Сортировка. Хранит состояние сортировки родительской таблицы. Поскольку, только у родительской таблицы есть заголовок, только она и меняет сортировку. Дочерние таблицы подписываются на изменения.
