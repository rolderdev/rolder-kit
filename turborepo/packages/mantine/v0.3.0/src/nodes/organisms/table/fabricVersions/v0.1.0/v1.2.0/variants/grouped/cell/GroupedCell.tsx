...
import { ActionIcon, Checkbox, Group, createStyles } from "@mantine/core";
...
export default function (
    ...
    const { noodlNode, multiSelectable, multiSelectCheckboxColor, tableDensity } = tableProps
    ...
    const actions = leafColumnDef.actions?.map(i => { return useAction(tableProps, i, row) })
    ...
    return <Group ml={margins.cell} position={position} noWrap>
        <Group noWrap>
            {row.getIsGrouped() && tableProps.expendOn === 'row'
                ? <IconChevronDown
                    size={20}
            ...
            />}
            {Value(groupScheme || leafColumnDef, value, leafColumnDef.size || tableProps.defaultColumnSize)}
        </Group>
        {!row.getIsGrouped() && leafColumnDef.hoverableActions
            ? hoveredRowId === row.id && actions
            : !row.getIsGrouped() && !tableProps.tableLoading && actions}
    </Group >
}