...
export default function (columnDef: ColumnDef, row: RItem) {
    switch (columnDef.data?.type) {
        ...
        case 'mask': return getMasked2(
            { type: 'pattern', maskPattern: columnDef.data.maskFormat },
            getValue8(row, columnDef.accessor, columnDef.data.default, columnDef.data.defaultAccessor)
        )
        case 'number': return numbro(getValue8(row, columnDef.accessor, columnDef.data.default, columnDef.data.defaultAccessor) || 0)
        ...
        default: return getValue8(row, columnDef.accessor, columnDef.data?.default, columnDef.data?.defaultAccessor)
    }
}