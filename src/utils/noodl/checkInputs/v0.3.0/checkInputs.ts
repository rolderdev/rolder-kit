...
export default function (thisProps: { noodlNode: NodeInstance, inputs?: NodePort2[], resultProps: any, hasFormContext?: boolean }): number {
    ...
    function checkHasValue(input: NodePort2) {
        const value = resultProps[input.name]
    ...
    if (inputs) {
        ...
        inputs.filter(i => i.required).forEach(input => {
            let dependsOnInput = undefined
            if (typeOf(input?.dependsOn) === 'string') dependsOnInput = inputs.find(i => i.name === input.dependsOn?.name)
            else if (typeOf(input?.dependsOn) === 'object') {
                const dependsOnName: any = input?.dependsOn
                dependsOnInput = inputs.find(i => i.name === dependsOnName?.input)
            }
