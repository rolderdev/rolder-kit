...
export const PortDef = pipe(
	object({
		name: string(),
		displayName: string(),
		group: picklist(['Version', 'Data', 'Params', 'Styles', 'Signals', 'Advanced', 'Custom']), // Строгий список, чтобы не опечатываться.
		...
		default: optional(union([string(), number()])),
		...
		visibleAt: optional(picklist(['editor', 'connection', 'both']), 'both'),
	...
	brand('PortDef') // Запрещает использовать тип напрямую без парсинга схемы.
);
...
export type PortDef = Omit<InferInput<typeof PortDef>, 'dependsOn' | 'validate' | 'transform'> & {
...
export const getPortDef = (portDef: PortDef) => parse(PortDef, portDef);
