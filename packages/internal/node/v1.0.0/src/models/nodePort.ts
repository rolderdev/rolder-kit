...
import { getNodePort, type NodePort, type PortDef } from '@packages/port';
...
// Устанавливает в дефолтные параметры undefined. Это создает порты для дефолтов, иначе не отработают проверки.
/* export const triggerDefaultPosrts = (node: GraphModelNode, versions: JsVersions) => {
	const nodeDef = versions[node.parameters.version];
	let nodeInputs = nodeDef?.inputs?.map((i) => ({ ...getNodePort('input', i) })) || [];

	for (const nodeInput of nodeInputs) {
		if (node.parameters[nodeInput.name] === undefined && nodeInput.default !== undefined)
			node.parameters[nodeInput.name] = undefined; // Тригерит
	}
};
 */
