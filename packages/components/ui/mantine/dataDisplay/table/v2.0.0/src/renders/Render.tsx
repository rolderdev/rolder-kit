...
export default memo((p: { expansionEnabled: boolean; column: Column; item: Item }) => {
	if (p.expansionEnabled && p.column.idx === 0) {
		...
		if (p.column.type === 'getValue' && p.column.getValue)
			return <ExpanderCell itemId={p.item.id} cell={<GetValueCell itemId={p.item.id} columnIdx={p.column.idx} />} />;
		...
		if (p.column.type === 'getValue' && p.column.getValue) return <GetValueCell itemId={p.item.id} columnIdx={p.column.idx} />;
