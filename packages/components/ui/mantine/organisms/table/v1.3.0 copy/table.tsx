                    ...
                    expanded: {
                        ...
                        onRecordIdsChange: (recordIds: any[]) => {
                            const createExpansionNode = async () => {
                                ...
                                const newExpansionRows = await Promise.all(newRecordIds.map(newRecordId => {
                                    const item = items.find(i => i.id === newRecordId)
                                    if (item) {
                                        const group = props.noodlNode.nodeScope.createPrimitiveNode('Group');
                                        group.setStyle({ flex: '1 0 100%' });
                                        return props.noodlNode.nodeScope.createNode(p.expansionTemplate, guid(), {
                                            _forEachModel: window.Noodl.Object.create(item),
                                            _forEachNode: props.noodlNode
                                        }).then((node: any) => {
                                            group.addChild(node);
                                ...
                                setExpansionRows([
                                    ...expansionRows.filter(i => !deletedRecordIds.includes(i.id)),
                                    ...newExpansionRows
                                ])
