...
function invalidateQueriesWithRetries(maxRetries: number, retryTimeout: number) {
    ...
    else timeout(R.libs.queryClient?.invalidateQueries(), retryTimeout)
        .then(() => retry = 1)
        .catch(() => {
            retry++
...
export default function (noodlNode: NoodlNode, offlineLatancy: number, offlineJitter: number, offlineDownload: number) {
    ...
    if (!speedTest.onFinish) speedTest.onFinish = r => {
        const { latency, jitter, download, upload } = r.getSummary()

        //@ts-ignore
        sendOutput(noodlNode, 'network', { latency: latency || 0, jitter: jitter || 0, download: download || 0, upload: upload || 0 })
    ...
    if (!speedTest.onError) speedTest.onError = () => {
        if (isOnline) {
            setOfflineHandler(noodlNode)
            setOnline(false)
        }
