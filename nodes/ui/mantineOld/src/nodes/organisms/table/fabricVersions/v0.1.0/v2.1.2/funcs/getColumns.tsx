...
import { Dispatch, cloneElement, useMemo } from "react";
        ...
        column.render = (record, idx) => {
            ...
            if (columnCell) {
                ...
                return expansion.enabled && column.expander
                    ? <Box {...boxProps}>{Expander(record.id, cloneElement(columnCell))}</Box>
                    : <Box {...boxProps}>{cloneElement(columnCell)}</Box>
            } else return expansion.enabled && column.expander
                ? <Box {...boxProps}>{Expander(record.id, customRender
                    ? customRender(record, idx)
                    : window.R.utils.getValue.v8(record, column.accessor))}</Box>
                : <Box {...boxProps}>{customRender
                    ? customRender(record, idx)
                    : window.R.utils.getValue.v8(record, column.accessor)}</Box>
        }
