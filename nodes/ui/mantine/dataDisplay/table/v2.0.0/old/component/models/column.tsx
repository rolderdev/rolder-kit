				...
				({
					...
					cellsStyle: () => {
						...
						if (snap.hierarchy.isChild && snap.libProps.withRowBorders) {
							...
							if (!snap.expandedIds[R.libs.just.last(snap.records.map((i) => i.id))]) return style
						}
						return
					},
					render: (record) =>
					...
					filtering: i.filter ? snap.filters.state?.[i.idx]?.enabled : false,
