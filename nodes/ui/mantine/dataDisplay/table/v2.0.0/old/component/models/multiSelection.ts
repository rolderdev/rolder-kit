	...
	if (tableNode) {
		for (const ancestorNode of tableNode.ancestorNodes(true)) {
			...
			if (ancestorState) {
				...
				const selectedChildrenCount = ancestorNode.childNodes().filter((i) => i.states.multiSelection.value === 'selected').length
...
export const useHierarchySelection = (s: Store) => {
	...
	useEffect(() => {
		return () => {
			for (const unsub of Object.values(s.checkboxes.unsubs)) unsub()
		}
	}, [])
}
