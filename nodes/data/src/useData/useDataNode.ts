...
export default reactNode(
	'UseData',
	{
		...
		'v0.12.3': {
			...
			inputs: [
				...
				getPort({
					plug: 'input',
					name: 'backRefs',
					displayName: 'Backward references',
					group: 'Backward references',
					type: getType('proplist', 'editor'),
					customs: {
						dependsOn(p) {
							return p.useDataContext === true;
		...
		'v1.0.0': {
			...
			inputs: [
				...
				getPort({
					...
					displayName: 'Pagination DB class',
					group: 'Pagination',
					type: 'string',
					customs: {
						required: 'connection',
		...
		'v1.4.0': {
			hashTag: '#expreimental',
			module: { dynamic: lazy(() => import('@packages/use-data-v1.4.0')) },
			inputs: [
				getPort({
					plug: 'input',
					name: 'apiVersion',
