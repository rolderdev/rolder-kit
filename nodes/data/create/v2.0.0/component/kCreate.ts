...
export default async (p: Props, createScheme: CreateScheme) => {
	...
	log.info(`create props: ${createScheme.map((i) => getDbClassName(i.dbClass)).join(', ')}`, p);
	...
	log.info(`create: ${createScheme?.map((i) => getDbClassName(i.dbClass)).join(', ')}`, backendData);
	log.end(`create: ${createScheme?.map((i) => getDbClassName(i.dbClass)).join(', ')}`, startTime);
