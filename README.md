...
1. `inNode`: запускается при каждом изменении инпутов. Здесь запускаются все обработки для каждого порта.
2. `afterNode`: запускается при каждом изменении инпутов после их обработки.
3. `beforeComponent`: запускается после всех `*node` и перед запуском (монтированием) компоненты.
