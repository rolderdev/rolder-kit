...
- `initialize`. Функция для одноразового запуска любого кода до первого запуска JS-ноды или первого рендера React-ноды. Принимает `props`, нужно вернуть их же. Код может быть асинхронным. Эта функция обеспечивает важный сценарий, когда нужно гарантировать не пустой возврат при первом запуске, как это сделано в нодах `item` и `node`.
