...
##### Очередность запусков следующая:

- RoodlTime (когда нода регистрируется в проекте):
  - Функции портов
  - transform на всю декларацию
  - validate на всю декларацию
- RunTime (когда код компоненты исполняется):
  - initialize
  - код компоненты
