	...
	({
			...
			registerInputIfNeeded: function (inputName: string) {
				...
				this.registerInput(inputName, {
					set: function (value: unknown) {
						console.log('registerInputIfNeeded', nodeName, inputName, value, this);
						...
						if (nodeDef && inputDef) {
							...
							if (this._hasInputBeenSetFromAConnection(inputName)) {
								...
								if (this.firstRun || this.model.firstRun || !nodeDef.transform) {
									// Сохраним декларацию портов в кеше, чтобы функции могли доверять ей, если есть transform.
		...
		setup: function (context: NodeContext, graphModel: GraphModel) {
			...
			graphModel.on(`nodeAdded.rolder-kit.api-v1.${nodeName}`, function (model: GraphModelNode) {
				...
				let versionPortDef = getVersionPortDef(versions); // Порты, которые нужные в кеше сразу.
				model.portDefsCache.inputs = [versionPortDef];
				...
				if (!model.parameters.version) {
					context.editorConnection.sendDynamicPorts(model.id, [getNodePort('input', versionPortDef)]);
				...
				model.on('parameterUpdated', function (port) {
					if (port.name === 'version') {
						...
						if (port.value === undefined) {
							...
							/* cachePortDefs({ model } as any, versions);
							//const inputs = model.portDefsCache?.inputs.map((i) => getNodePort('input', i)) || [];
							//const outputs = model.portDefsCache?.inputs.map((i) => getNodePort('output', i)) || [];
							setValuesFromParameters({ model } as any);
							setNodePorts({ model } as any); */
							model.firstRun = true;
						} // После установки версии нужно запустить модуль первый раз.
					}
				});
			});
		},
	...
	return {
		...
		getReactComponent() {
			return forwardRef(function (p: Props, ref) {
				console.log('forwardRef', nodeName, p);
