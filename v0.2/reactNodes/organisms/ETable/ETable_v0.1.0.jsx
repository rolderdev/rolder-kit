...
import { getValue, filterBy } from '../../../utils/data/v.0.1.0/data'
...
export default function ETable_v0_1_0(props) {
  ...
  useShallowEffect(() => {
    ...
    if (tableData) {
      ...
      if (selectedFilters.length) {
        setSelectedData([])
        filteredData.task = []
        tableData.task.forEach(task => {
          let count = 0
          selectedFilters.forEach(filter => {
            const contentType = tableScheme[0].columns.find(c => c.filterScheme?.valueField === filter).filterScheme.contentType
            switch (contentType) {
          ...
          case 'MultiSelect':
            column.filter = (
              <Box w={filterProps.props.width}>
                <MultiSelect
                  data={tableData?.[filterProps.valueField].map(item => ({ value: item.id, label: getValue(item, filterProps.labelField) }))}
