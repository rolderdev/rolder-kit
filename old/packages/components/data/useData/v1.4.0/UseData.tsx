	...
	if (scheme.hierarchy) {
		...
		if (!scheme.hierarchy.scheme) {
			...
			R.libs.mantine?.MantineError?.(
				'Системная ошибка!',
				`No dbClass at hierarchy scheme: ${JSON.stringify(scheme.hierarchy.scheme)}`
			);
...
export default forwardRef(function (props: Props, ref) {
	...
	const fetchScheme =
		props.fetchScheme &&
		getScheme(props.fetchScheme, paginationScheme, pagesSearchAfter.find((i) => i.page === page)?.searchAfter);
	if (!fetchScheme) {
		sendOutputs(noodlNode, [
			{ portName: 'error', value: true },
			{ portName: 'fetching', value: false },
		]);
		return null;
	}
	...
	useEffect(() => {
		...
		if (data && !isFetching) {
			let ouputs: any = Object.keys(data)
				.map((dbClass) => [
