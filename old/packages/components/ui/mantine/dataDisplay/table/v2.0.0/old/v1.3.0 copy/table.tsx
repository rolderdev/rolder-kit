...
export default forwardRef((props: Props, ref) => {
	...
	useEffect(() => {
		if (tableId !== '' && tableId !== undefined) {
				...
				items.forEach((item) => {
					if (item && parentTableItemId && tableSelectionScopeValue['tableSelectionScope'][item.id] === undefined) {
						...
						if (tableSelectionScopeValue['tableSelectionScope'][parentTableItemId] !== 'indeterminated') {
							tableSelectionScopeValue['tableSelectionScope'][item.id] =
								tableSelectionScopeValue['tableSelectionScope'][parentTableItemId]
						}
			...
			tableSelectionScopeValue['forRenderTableId'] = {
				...
				childTableId: Object.keys(tableSelectionScopeValue['parentTableIdByTableId']).filter(
					(key) => tableSelectionScopeValue['parentTableIdByTableId'][key] === tableId
				),
			}
		...
		return () => {
			...
			tableSelectionScopeValue['allTableIdList'] = tableSelectionScopeValue['allTableIdList'].filter(
	...
	const itemsInScope =
		items.length > 0 ? (tableSelectionScopeValue['tableSelectionScope'][items[0].id] !== undefined ? true : false) : false
	...
	if (items.length === 0) {
		tableSelectionScopeValue['forRenderTableId'] = {
			...
			currentTableId:
				tableId === tableSelectionScopeValue['forRenderTableId']['currentTableId']
					? undefined
					: tableSelectionScopeValue['forRenderTableId']['currentTableId'],
			newTableId:
				tableId === tableSelectionScopeValue['forRenderTableId']['newTableId']
					? undefined
	...
	const refreshScopeValuesBySelect = () => {
		...
		if (useScopeStates) {
			...
			items.forEach((item) => {
				...
				if (tableSelectionScopeItem === 'selected') {
					...
					if (tableSelectionScopeValue['tableIndeterminatedItemsIdList'].includes(item.id)) {
						tableSelectionScopeValue['tableIndeterminatedItemsIdList'] = tableSelectionScopeValue[
			...
			// Если id текущей таблицы дочернее, то записываем дочерние
				...
				tableSelectionScopeValue['forRenderTableId']['childTableId'] = tableSelectionScopeValue['forRenderTableId'][
					'childTableId'
				]?.filter((iChildTableId) => iChildTableId !== tableId)
				const childIds = Object.keys(tableSelectionScopeValue['parentTableIdByTableId']).filter(
					(key) => tableSelectionScopeValue['parentTableIdByTableId'][key] === tableId
				)
				tableSelectionScopeValue['forRenderTableId']['childTableId'] = [
					...tableSelectionScopeValue['forRenderTableId']['childTableId'],
			...
			if (hasChange) {
				...
				tableSelectionScopeValue['selectionByTableId'][tableId] = newSelectedRecords
				setTableSelectionScopeValue(tableSelectionScopeValue)
			}
		}
	...
	/*     const expansionRow = useMemo(() => {
            ...
            return Array.isArray(ch)
                ? ch.filter(i => i.type?.name === 'ForEachComponent')?.[0]
                : ch?.type?.name === 'ForEachComponent'
                    ? ch
                    : null
        }, []) */

	// Ноды, готовые для рендеринга при разворачивании
	const [expansionRows, setExpansionRows] = React.useState<{ id: string; rowRender: any }[]>([])

	/* useEffect(() => {
        if (items?.length) {
            const createExpansionNodes = async () => {
                const nodes: any = await Promise.all(
                    items.map((item) => {
                        const group = props.noodlNode.nodeScope.createPrimitiveNode('Group');
                        group.setStyle({ flex: '1 0 100%' });
                        return props.noodlNode.nodeScope
                            .createNode('/App/TestTemplate', guid(), {
                                _forEachModel: window.Noodl.Object.create(item),
                                _forEachNode: props.noodlNode
                            })
                            .then((node: any) => {
                                group.addChild(node);
                ...
                showLoader.setKey(tableId, false)
                setTimeout(() => setExpansionCachedRows(nodes), 1000)
            };

            createExpansionNodes().catch(console.error);
        }
    }, [items, showLoader, tableId]);
 */
	// Columns
				...
				if (columnCell) {
					...
					return expansion.enabled && column.expander ? (
						<Box {...boxProps}>
							{Expander(
								record.id,
								cloneElement(columnCell, { innerProps: { record } }),
								expandedRecordIds,
	...
	//@ts-ignore
	return (
		<ScopeProvider scope={TableFilterScope} value={tableId}>
						...
						expansion.enabled
							? {
									...
									expanded: {
										...
										onRecordIdsChange: (recordIds: any[]) => {
											const createExpansionNode = async () => {
												const newRecordIds = recordIds.filter((i) => !expandedRecordIds.includes(i))
												const deletedRecordIds = expandedRecordIds.filter((i) => !recordIds.includes(i))
												const newExpansionRows = await Promise.all(
													newRecordIds.map((newRecordId) => {
														...
														if (item) {
															...
															group.setStyle({ flex: '1 0 100%' })
															return props.noodlNode.nodeScope
																.createNode(p.expansionTemplate, guid(), {
																	_forEachModel: window.Noodl.Object.create(item),
																	_forEachNode: props.noodlNode,
																})
																.then((node: any) => {
												...
												setExpansionRows([...expansionRows.filter((i) => !deletedRecordIds.includes(i.id)), ...newExpansionRows])
												setTimeout(() => setExpandedRecordIds(recordIds))
											}

											createExpansionNode().catch(console.error)
										},
									},
									content: ({ record }: any) => expansionRows.find((i) => i.id === record.id)?.rowRender,
						...
						cx(
							...
							{ [classes.striped]: rowStyles.enabled && rowStyles.striped },
							{
								[classes.multiSelected]:
									rowStyles.enabled && selection.multi.enabled && selectedRecords?.map((i) => i.id).includes(id),
							},
