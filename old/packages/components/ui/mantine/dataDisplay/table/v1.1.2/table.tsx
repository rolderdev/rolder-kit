...
import { TableScope, useTableScope } from '@packages/scope'
...
export default forwardRef((props: Props, ref) => {
		...
		return columnsDef.map((column, columnIdx) => {
			...
			if (!filter.enabled) {
				...
				if (columnFilter) {
					...
					column.filtering =
						getColumnFilter(columnIdx)?.state !== undefined ? getColumnFilter(columnIdx)?.state : column.filterDefaultEnabled
				}
			}
	...
	//@ts-ignore
	return tableId ? (
		<ScopeProvider scope={TableScope} value={tableId}>
