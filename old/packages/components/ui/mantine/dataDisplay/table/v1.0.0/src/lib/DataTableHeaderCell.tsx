...
import { type BaseSyntheticEvent, type CSSProperties, type ReactNode, useRef, useState } from 'react'
...
type DataTableHeaderCellProps<T> = {
	...
	DataTableColumn<T>,
	'accessor' | 'sortable' | 'draggable' | 'toggleable' | 'resizable' | 'textAlignment' | 'width' | 'filter' | 'filtering'
>
								...
								.map((column: DataTableColumnToggle) => {
									return (
										<Group key={column.accessor}>
											<Checkbox
												size="xs"
												label={allColumns.find((c) => c.accessor === column.accessor)?.title ?? humanize(column.accessor)}
