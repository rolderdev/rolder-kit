	...
	async createXlsx(props: Props) {
		...
		function accessorFn(columnDef: XlsxColumn, item: Item) {
			switch (columnDef.dataType) {
				case 'date':
					return getFormatedDate(item, columnDef.accessor, columnDef.dataFormat)
				case 'mask':
					return getMasked(getValue(item, columnDef.accessor, columnDef.defaultValue), columnDef.dataFormat)
				case 'number':
					return numbro(getValue(item, columnDef.accessor, columnDef.defaultValue) || 0).format(columnDef.dataFormat)
				default:
					return getValue(item, columnDef.accessor, columnDef.defaultValue)
			}
		}

		const xlsxData = items?.map((item) => {
			const xlsxItem: { [x: string]: any } = {}
