...
import type { DbClass, User } from 'types'
...
export async function prepData() {
	...
	try {
		...
		dbClassesResp.hits.map((i) => {
			dbClasses[i._source.name] = i._source as any
		})
		...
		log.error('Get DB classes error', e)
	}
	...
	try {
		...
		if (user._source.dbClass) {
			try {
				...
				log.error('Get user DB class error', e)
			}
		} else {
		...
		log.error('Get user system DB class error', e)
	}
